...
trait ExternalCallIntegrationTestBase {
  ...
  /** Configuration transform to enable external call extension on a participant.
    *
    * @param extensionId
    *   The extension ID to configure (e.g., "test-ext")
    * @param port
    *   The port of the mock HTTP server
    * @param participantName
    *   The participant to configure (e.g., "participant1")
    */
  ...
  /** Initialize the mock server. Should be called in test setup (e.g., in withSetup block).
    */
  ...
  /** Shutdown the mock server. Should be called in afterAll.
    */
...
/** Mixin for tests that need the mock server started before all tests. This trait should be mixed
  * into concrete test classes that also extend SharedEnvironment or IsolatedEnvironments.
  */
